...
export class MediaService {
  ...
  async transcribeAudio(mediaId: string): Promise<string> {
    ...
    const mediaUrl =
      await this.whatsappService.getMediaUrl(mediaId);
    const audioBuffer =
    ...
    this.logger.log(
      `Audio transcribed: ${result.text.substring(0, 50)}...`,
    );
    return result.text;
  }
  ...
  async processAndStoreImage(
    ...
    this.logger.log(
      `Processing image ${mediaId} for ${subscriberId}`,
    );
    ...
    const key =
      `subscribers/${subscriberId}/${purpose}/` +
      `${Date.now()}.jpg`;
  ...
  async uploadToS3(
    ...
    return (
      `https://${this.s3Bucket}.s3.` +
      `${this.s3Region}.amazonaws.com/${key}`
    );
  }
  ...
  async uploadPdfToS3(
    ...
    const key =
      `subscribers/${subscriberId}/documents/${filename}`;
