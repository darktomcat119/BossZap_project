...
export function AppShell({ children }: AppShellProps) {
  ...
  // Connect to the WebSocket once at the shell level so live events
  // (notifications, dashboard updates) work on every authenticated page.
  useWebSocket();
      ...
      <div className="flex flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-surface px-4 md:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* User menu — avatar / business name / dropdown */}
            <UserMenu />
          </div>
        </header>

        {/* Main content. Bottom padding on mobile keeps content clear of
            the fixed bottom-nav (matches the nav's ~56px height). */}
